...
case "$1" in
    ...
    *)
        ...
        ~/.config/polybar/scripts/system_usb_udev.sh --monitor &
...
#ACTION=="add|remove", SUBSYSTEM=="block", ENV{DEVTYPE}=="partition", RUN+="/bin/su SEU_USUARIO -c '/bin/sh -c \"~/.config/polybar/scripts/system_usb_udev.sh --update\"'"
