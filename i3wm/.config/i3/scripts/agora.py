...
class BellSchedulerApp(Gtk.Application):
    ...
    def criar_janela_principal(self):
        ...
        # Botão Procurar
        btn_som = Gtk.Button.new_from_icon_name("folder-symbolic", Gtk.IconSize.BUTTON)
        btn_som.set_tooltip_text("Procurar arquivo de som")
        btn_som.connect("clicked", self.ao_procurar_som)
        
        # Botão Testar
        self.btn_testar = Gtk.Button.new_from_icon_name("audio-volume-high-symbolic", Gtk.IconSize.BUTTON)
        self.btn_testar.set_tooltip_text("Testar som")
        self.btn_testar.connect("clicked", self.ao_testar_som)
        self.btn_testar.get_style_context().add_class("sound-button")
        
        # Botão Parar Som Atual
        self.btn_parar_som = Gtk.Button.new_from_icon_name("audio-volume-muted-symbolic", Gtk.IconSize.BUTTON)
        self.btn_parar_som.set_tooltip_text("Parar som atual")
        self.btn_parar_som.connect("clicked", self.ao_parar_som_atual)
        self.btn_parar_som.get_style_context().add_class("sound-button")
        self.btn_parar_som.set_sensitive(False)  # Inicia desativado
        
        som_box.pack_start(self.entrada_som, True, True, 0)
        som_box.pack_start(btn_som, False, False, 0)
        som_box.pack_start(self.btn_testar, False, False, 0)
        som_box.pack_start(self.btn_parar_som, False, False, 0)
        ...
        btn_icone = Gtk.Button.new_from_icon_name("folder-symbolic", Gtk.IconSize.BUTTON)
        btn_icone.set_tooltip_text("Procurar arquivo de ícone")
        btn_icone.connect("clicked", self.ao_procurar_icone)
        btn_preview_icone = Gtk.Button.new_from_icon_name("image-x-generic-symbolic", Gtk.IconSize.BUTTON)
        btn_preview_icone.set_tooltip_text("Visualizar ícone")
    ...
    def ao_testar_som(self, widget):
        ...
        if self.is_sound_playing:
            self.mostrar_info("Som em reprodução", "Já há um som sendo reproduzido")
            return
            
        sound_file = self.entrada_som.get_text()
        if not self.validar_caminho_arquivo(sound_file):
            self.mostrar_erro("Arquivo inválido", "O arquivo de som não pode ser acessado")
            return

        try:
            self.is_sound_playing = True
            self.btn_testar.set_sensitive(False)
            self.btn_parar_som.set_sensitive(True)
            self.btn_parar_som.set_image(Gtk.Image.new_from_icon_name("media-playback-stop-symbolic", Gtk.IconSize.BUTTON))
            
            self.current_sound_process = subprocess.Popen(["paplay", sound_file])
            
            # Monitora o processo em segundo plano
            def monitorar_som():
                self.current_sound_process.wait()
                GLib.idle_add(self.ao_som_terminado)
                
            threading.Thread(target=monitorar_som, daemon=True).start()
            
        except Exception as e:
            self.mostrar_erro("Erro ao reproduzir som", str(e))
            self.is_sound_playing = False
            self.btn_testar.set_sensitive(True)
            self.btn_parar_som.set_sensitive(False)

    def ao_som_terminado(self):
        """Chamado quando o som termina de tocar"""
        self.is_sound_playing = False
        self.current_sound_process = None
        self.btn_testar.set_sensitive(True)
        self.btn_parar_som.set_sensitive(False)
        self.btn_parar_som.set_image(Gtk.Image.new_from_icon_name("audio-volume-muted-symbolic", Gtk.IconSize.BUTTON))
