...
XPATH_MENU_FICHA      = "//*[self::a or self::td or self::li or self::div or self::span][normalize-space(.)='Ficha individual']"
...
def clicar(driver, wait, xpath, descricao, scroll=True):
    # Melhoria: trata StaleElementReferenceException com 3 tentativas
    for tentativa in range(3):
        try:
            ...
            print(f"    ⚠️  Elemento stale, tentativa {tentativa+1}/3...")
            time.sleep(0.5)
        except TimeoutException:
            driver.save_screenshot(f"erro_{descricao.replace(' ','_')}.png")
            raise RuntimeError(f"Timeout: '{descricao}'\nXPath: {xpath}")
    raise RuntimeError(f"'{descricao}' ficou stale após 3 tentativas.")
...
def selecionar_select(driver, xpath, valor, descricao, pausa=2.0, por_texto=False):
    # Idêntico ao funcional + tratamento de StaleElement
...
def clicar_direto(driver, xpath, descricao, pausa=2.0, scroll=True):
    # Melhoria: trata StaleElement entre find e click
    deadline = time.time() + 20
    while time.time() < deadline:
        els = driver.find_elements(By.XPATH, xpath)
        if els:
...
def ler_turmas_disponiveis(driver, xpath_select):
    # Melhoria: timeout aumentado para 20s (AJAX em cascata pode demorar)
    deadline = time.time() + 20
    while time.time() < deadline:
        ...
        if els:
            try:
                ...
                pass
        time.sleep(0.5)
    driver.save_screenshot("erro_turmas_nao_carregaram.png")
    raise RuntimeError(
        "Turmas não carregaram após seleção da Série.\n"
...
try:
    ...
    input()
    _t_inicio = time.time()
    print("⏳ Iniciando automação — FICHA INDIVIDUAL...\n")
    ...
    clicar_em_iframe(driver,
        "//*[self::a or self::td or self::li or self::div or self::span][normalize-space(.)='Alunos']",
        "Menu Alunos", pausa=2)

    print("5️⃣  Clicando em 'Documentos'...")
    clicar_em_iframe(driver,
        "//*[self::a or self::td or self::li or self::div or self::span][normalize-space(.)='Documentos']",
        "Submenu Documentos", pausa=2)

    print("6️⃣  Clicando em 'Ficha individual'...")
    clicar_em_iframe(driver,
        "//*[self::a or self::td or self::li or self::div or self::span][normalize-space(.)='Ficha individual']",
        "Ficha individual", pausa=3)
    ...
    print("\n" + "═"*50)
    print("🏆 FICHA INDIVIDUAL — AUTOMAÇÃO CONCLUÍDA!")
    print(f"   Curso : {CURSO}")
    ...
    diagnosticar(driver, "ficha_individual")
    ...
    print(f"\n❌ Erro inesperado: {e}")
    print(f"   ⏱️  Tempo até o erro: {_min}m {_seg:02d}s")
    print("   → Screenshot: erro_ficha_individual.png")
    input("\nPressione ENTER para encerrar...")
