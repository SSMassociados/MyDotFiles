...
def export_results_to_pdf():
    ...
    if filename:
        ...
        messagebox.showinfo("Exportação Cancelada", "Nenhum arquivo selecionado. A exportação foi cancelada.")
...
def open_presence_document_thread():
    presence_document_path = "/home/sidiclei/Área de trabalho/AUSTRO/DECLARAÇÕES/DECLARAÇÃO DE COMPARECIMENTO.docx"
    if os.path.exists(presence_document_path):
        subprocess.run(["xdg-open", presence_document_path])
    else:
        messagebox.showerror("Erro", "O documento de presença não foi encontrado.")

# Definir funções que chamam as funções em threads separadas

def open_declaration():
    threading.Thread(target=open_declaration_thread).start()

def open_presence_document():
    threading.Thread(target=open_presence_document_thread).start()
...
def show_search_result(event=None):
    ...
    entry_student.select_range(0, tk.END)  # Seleciona todo o texto na entrada de texto
...
if __name__ == "__main__":
    ...
    presence_button = tk.Button(export_buttons_frame, text="Presença", font=('JetBrains Mono', 12), command=open_presence_document, bg='#007acc', fg='#ffffff', width=11)
