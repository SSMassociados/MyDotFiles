...
while true; do
    ...
    battery_level=$(upower -i /org/freedesktop/UPower/devices/battery_BAT0 | grep -E "percentage" | awk '{print $2}' | tr -d '%')
    
    # Verificar se o carregador está conectado
    if [ "$(upower -i /org/freedesktop/UPower/devices/battery_BAT0 | grep -c "state: *charging")" -eq 1 ]; then
