...
echo "MATRÍCULA,NOME CIVIL,DATA DE NASCIMENTO,CIDADE,UF,FILIAÇÃO 1,FILIAÇÃO 2,ANO/FASE/MÓDULO,TURMA,TURNO,E-MAIL,TELEFONE" > "$saida"
    ...
    filiacao_2="${filiacao_2:-Desconhecido}"
            ...
            echo "\"${matricula}\",\"${nome_civil}\",\"${data_nascimento}\",\"${cidade}\",\"${uf}\",\"${filiacao_1}\",\"${filiacao_2}\",\"${ano_fase_modulo}\",\"${turma}\",\"${turno}\",\"${email}\",\"${telefone}\"" >> "$saida"
